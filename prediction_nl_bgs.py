...
class FitParams:
    ...
    """
    num_pulls: ClassVar = 6 + 8 + 8 + 8 + 3 + 3 + 1 + 8 + 8 + 1 + 1 + 1 + 4 * 34 + len(nl_pulls) + 8  # TODO binning
    theta13: float
    m2_ee: float
    ...
    pulls_IAV: dict = dc_field(default_factory=lambda: ad_dict(0))
...
#        print("INDEX MAP")
        ...
        to_return['IAV'] = slice(58 + num_near_stat + len(nl_pulls), 58 + num_near_stat + len(nl_pulls) + 8)
        return to_return
...
#        print("What about here?", pulls_nonlinearity)
        pulls_IAV = {}
        for pull, halldet in zip(in_list[indexes['IAV']], all_ads):
            pulls_IAV[halldet] = pull
        return cls(
            theta13,
            m2_ee,
            pull_reactor,
...
#        print("Lookie here: ", self.pulls_nonlinearity)
        ...
        to_return = (
            ...
            + [self.pulls_IAV[halldet] for halldet in all_ads]
        )
...
def load_constants(config_file, json_str=False):
    ...
    if config.masses is True:
        ...
        raise ValueError(f"Invalid masses specification: {config.masses}")

    # Parse efficiency_err: Nominal, 1, hard-coded, or alternate database
    if isinstance(config.efficiency_error, list):
        efficiency_err = zip_ads(config.efficiency_error)

    else:
        raise ValueError(f"Invalid efficiency_error specification: {config.efficiency_error}")
    ...
    # Need to update as the IAV pulls!!! FIXME HODGEPODGE
    IAV_params = IAV_parameters(
        database, "THU ToyMC", config.binning_id
    )
...
def IAV_parameters(database, source, binning_id):
    """Load the shape distortion parameters for the IAV pull terms.

    Return a dict keyed by hall, detector, and pull_id
    whose values are arrays with rows corresponding to reco bins and columns
    corresponding to 0: distortion for plus, 1: distortion for minus.
    The scale of the distortions is normalized so that the factor
    (1 + IAV_offset * IAV_parameters) gives the appropriate
    correction if IAV_offset is e.g. 0.001 for 0.1%.
    """
    result = {}
    with common.get_db(database) as conn:
        cursor = conn.cursor()
        ...
        params = np.array(cursor.fetchall())
        result = params[:, :2]/params[:, 2:]  # divide by last column (sigmas)
#        print("IAV PULL CURVES", result)
    return result
...
def efficiency_weighted_counts(constants, fit_params, halls='all'):
    ...
    to_return = {}
    IAV_params = constants.IAV_parameters[:, 0]
#    print("IAV PARAMS: ", IAV_params)
    for halldet, coincidences in num_coincidences.items():
        ...
        pull_IAV = fit_params.pulls_IAV[halldet]
        hall, det = halldet
...
#        print("Testing it in location 1:", fit_params.pulls_nonlinearity)
        ...
        combined_eff = (
            ...
            * (1 + pull_IAV * IAV_params) #apply the IAV shape change
        )
...
#            print("ERRORS", li9_errors)
#            print("Li9 relative uncertainty at the start:", li9_errors)
...
#            print("Fast neutron relative uncertainty at the start:", fast_neutron_errors)
        if 'amc' in types:
            for spectra_source in spectra_sources:
                cursor.execute('''
                    SELECT
...
#    print(nominal_bg)
    for bg in bg_types:
        ...
        if bg == 'accidental': bg_pulls['accidental'] = fit_params.pull_accidental
        if bg == 'li9': bg_pulls['li9'] = fit_params.pull_li9
        if bg == 'fast-neutron': bg_pulls['fast-neutron'] = fit_params.pull_fast_neutron
        if bg == 'alpha-n': bg_pulls['alpha-n'] = fit_params.pull_alpha_n
        if bg == 'rad-n': bg_pulls['rad-n'] = fit_params.pull_rad_n
        bg_specs = nominal_bg[bg]
        bg_dict = {}
        for hall, det in ads:
            bg_spec = bg_specs[hall, det]
            if bg == 'li9' or bg == 'fast-neutron': pull_key = hall #For those correlated within sites, but uncorrelated between sites
            else: pull_key = (hall, det) #For those uncorrelated between all ADs
            pull = bg_pulls[bg][pull_key]
            bg_dict[hall, det] = bg_spec * (1 + pull)
        pulled_bg[bg] = bg_dict
    

    if 'amc' in nominal_bg: #Only one pull term, not a dictionary, therefore applied separately
...
#        fit_params.pulls_IAV[far_halldet] = 1
        pull_IAV = fit_params.pulls_IAV[far_halldet]
...
#    print(predict_ad_to_ad_obs(constants, fit_params))
    print("Pull_li9",fit_params.pull_li9)
