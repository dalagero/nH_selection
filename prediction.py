...
#Getting the values
    ...
    pred_far_reco = predict_ad_to_ad_obs(constants, fit_params)
    data = constants.observed_candidates
#Bins
    true_bins = constants.true_bins_response
    reco_bins = constants.reco_bins
    ...
    for h, fardet in enumerate(common.far_ads):
        for i_reco in range(0,len(reco_bin_centers)):
...
#Filling the histograms
            for h, halldet in enumerate(common.near_ads):
                h_reco_near.Fill(reco_bin_centers[i_reco], (num_coincidences[halldet[0],halldet[1]][i_reco])/4.)
                for i_true in range(0,len(true_bins)-1):
                    h_true_energy_near.Fill(true_bin_centers[i_true], (true_energy_near[halldet[0],halldet[1]][i_true,i_reco])/4.)
                    for i_core in range(0,6):
                        h_true_energy_near_core[i_core].Fill(true_bin_centers[i_true], (true_energy_near_core[(halldet[0],halldet[1]),i_core+1][i_true,i_reco])/4.)
                        h_true_energy_far_core[i_core].Fill(true_bin_centers[i_true], (true_energy_far_core[(fardet[0],fardet[1]),i_core+1,(halldet[0],halldet[1])][i_true,i_reco]))
                        h_true_energy_far.Fill(true_bin_centers[i_true], (true_energy_far_core[(fardet[0],fardet[1]),i_core+1,(halldet[0],halldet[1])][i_true,i_reco]))
                h_reco_far_pred.Fill(reco_bin_centers[i_reco], (pred_far_reco[(fardet[0],fardet[1]),(halldet[0],halldet[1])][i_reco])/4)
            h_reco_far.Fill(reco_bin_centers[i_reco], (data[fardet[0],fardet[1]][i_reco])) #CHANGE IT HERE
            for i_core in range(0,6):
...
#    fig_flowchart = plot_flowchart(constants, fit_params)
...
#    fig_prompt_sub.tight_layout()
    ...
    plt.show()
    print("Input parameters", fit_params)
